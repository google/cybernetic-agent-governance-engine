...
1. **Internal identity leakage (HIGH)** — The GCP project ID `laah-cybernetics`, the GitHub username `lahlfors`, and the developer's local filesystem path `/Users/larsahlfors/` appear in 66+ locations across Markdown docs, YAML deployment manifests, Terraform files, and two scripts. These are the most urgent items because they expose the author's personal identity and internal GCP project name.
...
4. **`deployment/` and `compliance/` YAML files (MEDIUM)** — Kubernetes manifests, Cloud Build configs, and compliance audit evidence files contain hardcoded `gcr.io/laah-cybernetics/` image references, `laah-cybernetics` GCP project IDs, and GCS bucket names. These must be parameterised before public release.
...
6. **`README.md` internal cluster reference (LOW)** — Line 21 names the internal GKE cluster `gke_laah-cybernetics_us-central1-a_cage-dev`. This must be replaced with a generic placeholder.
...
| # | Location | Issue | Fix |
|---|----------|-------|-----|
| R-01 | Line 21 | `gke_laah-cybernetics_us-central1-a_cage-dev` — internal cluster name exposed | Replace with: `Tests run against a live GKE cluster. See infra/DEPLOYMENT_GUIDE.md for cluster setup.` |
...
### 6.1 Internal GCP Project ID: `laah-cybernetics`
...
| File | Line(s) | Pattern |
|------|---------|---------|
| `deployment/k8s/gateway.yaml` | 22, 25, 42 | `gcr.io/laah-cybernetics/gateway:latest`, `GOOGLE_CLOUD_PROJECT: "laah-cybernetics"` |
...
| `deployment/k8s/service-account.yaml` | 21 | `financial-advisor-sa@laah-cybernetics.iam.gserviceaccount.com` |
| `deployment/k8s/security-scan-cronjob.yaml` | 49 | `gcr.io/laah-cybernetics/gateway:latest` |
| `deployment/k8s/sbom-cronjob.yaml` | 361 | `GCP_PROJECT_ID: "laah-cybernetics"` |
| `deployment/k8s/lula-cron.yaml` | 192, 195, 319 | Same pattern |
| `deployment/k8s/gcp/ingress-gke.yaml` | 15 | DNS zone reference |
| `deployment/docker/cloudbuild.advisor.yaml` | 31 | `_GCP_PROJECT_ID: "laah-cybernetics"` |
...
| `deployment/update_langfuse_secret.py` | 215 | `langfuse-instance-laah-cybernetics` |
...
**Fix strategy:**
1. In all Kubernetes manifests and Cloud Build configs: replace `gcr.io/laah-cybernetics/` with `${IMAGE_REGISTRY}/` (already partially done with comments in some files) and `laah-cybernetics` project ID with `${GCP_PROJECT_ID}` substitution variable.
...
Line 215 contains `langfuse-instance-laah-cybernetics` as a hardcoded fallback. Replace with `langfuse-instance-<project-id>` or read from an environment variable.

### 6.5 Hardcoded GCS Model Paths

Several Kubernetes manifests hardcode GCS paths like `gs://laah-cybernetics-models/models--Qwen--...`. These expose the internal GCS bucket name and specific model snapshot SHA. Replace with environment variable references:

```yaml
# Before:
value: "gs://laah-cybernetics-models/models--Qwen--Qwen2.5-7B-Instruct/snapshots/a09a35..."
# After:
valueFrom:
  configMapKeyRef:
    name: model-config
...
| # | Task | Files | Mode |
|---|------|-------|------|
| 2.1 | **Parameterise all `gcr.io/laah-cybernetics/` image references** in Kubernetes manifests — replace with `${IMAGE_REGISTRY}/<service>:latest` | All `deployment/k8s/*.yaml` | Code |
| 2.2 | **Replace `laah-cybernetics` GCP project ID** in all K8s manifests and Cloud Build configs with `${GCP_PROJECT_ID}` substitution | `deployment/k8s/*.yaml`, `deployment/docker/cloudbuild.*.yaml` | Code |
| 2.3 | **Replace `gs://laah-cybernetics-models/` GCS paths** in K8s manifests with environment variable references | `deployment/k8s/gateway.yaml`, `financial-advisor.yaml`, `compliance-bridge.yaml` | Code |
| 2.4 | **Sanitize `docs/operations/RELEASE_RUNBOOK.md`** — replace all `/Users/larsahlfors/...` paths and `laah-cybernetics` project references with `<placeholder>` variables | `docs/operations/RELEASE_RUNBOOK.md` | Code |
...
| **Internal URL check** | `laah-cybernetics` references in `deployment/` and `docs/` | Resolved by Phase 1 and Phase 2 tasks above |
...
- name: Check for internal references
  run: |
    if grep -r "laah-cybernetics\|lahlfors\|larsahlfors\|/Users/lars" \
