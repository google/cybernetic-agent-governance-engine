...
2. **Implement actual `fetch_baseline()` network call** — an HTTP `GET` to a real FlowSignal baseline endpoint, following the `Provider01.fetch_baseline()` pattern ([`provider_01/provider.py:112-131`](../src/integrations/provider_01/provider.py:112)) as the template.
...
7. **FlowSignal production authentication mechanism.** Bearer token (`FLOWSIGNAL_API_KEY_SECRET`, matching the `Provider01`/`Provider03` pattern) vs. application-level mTLS (Section 2.4/2.6) — this depends on how the real FlowSignal production endpoint is deployed (behind the service mesh vs. external), which is outside this repository's visibility and must be confirmed with the FlowSignal team.
