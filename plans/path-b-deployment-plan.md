...
**Container registry**: `gcr.io/YOUR_GCP_PROJECT_ID/` (GCR) | Artifact Registry (lula only): `us-central1-docker.pkg.dev/YOUR_GCP_PROJECT_ID/governance-stack/`
...
- `ImagePullBackOff` or `ErrImagePull` → image tag issue (the CronJob uses `gcr.io/YOUR_GCP_PROJECT_ID/compliance-bridge:latest`)
...
The CronJob at [`deployment/k8s/lula-cron.yaml`](../deployment/k8s/lula-cron.yaml) line 191 uses `gcr.io/YOUR_GCP_PROJECT_ID/compliance-bridge:latest`.

```bash
# Check that the image exists and is accessible from the cluster's service account
gcloud container images list-tags gcr.io/YOUR_GCP_PROJECT_ID/compliance-bridge \
...
The deployment at [`deployment/k8s/lula-cron.yaml`](../deployment/k8s/lula-cron.yaml) line 303 references `us-central1-docker.pkg.dev/YOUR_GCP_PROJECT_ID/governance-stack/lula:0.9.0`.

```bash
gcloud artifacts docker images list \
  us-central1-docker.pkg.dev/YOUR_GCP_PROJECT_ID/governance-stack/lula \
  --project=YOUR_GCP_PROJECT_ID \
  --include-tags \
...
**Expected output patterns**:
- `Events`: `Failed to pull image "us-central1-docker.pkg.dev/YOUR_GCP_PROJECT_ID/governance-stack/lula:0.9.0": ... not found` → tag missing
...
**Expected**: `us-central1-docker.pkg.dev/YOUR_GCP_PROJECT_ID/governance-stack/lula:0.9.0`
...
gcloud builds triggers import \
  ...
  --source=<(gcloud builds triggers describe <trigger-name> \
    --project=YOUR_GCP_PROJECT_ID --format=json \
...
**Image**: `gcr.io/YOUR_GCP_PROJECT_ID/vllm-streamer:latest` (updated by Path B)
...
**Image**: `gcr.io/YOUR_GCP_PROJECT_ID/nemo-guardrails:latest` (updated by Path B)
**Timeout**: 5 minutes
...
**Deployment**: `gateway`
**Image**: `gcr.io/YOUR_GCP_PROJECT_ID/gateway:latest` (updated by Path B)
**Timeout**: 5 minutes
...
**Image**: `gcr.io/YOUR_GCP_PROJECT_ID/compliance-bridge:latest` (updated by Path B; also resolves lula-audit CronJob image if that was the root cause)
**Timeout**: 5 minutes
...
- Image shows `us-central1-docker.pkg.dev/YOUR_GCP_PROJECT_ID/governance-stack/lula:0.9.0`
...
kubectl set image deployment/<deployment-name> \
  <container-name>=gcr.io/YOUR_GCP_PROJECT_ID/<image-name>:${KNOWN_GOOD_SHA} \
  -n governance-stack
...
**Image**: `gcr.io/YOUR_GCP_PROJECT_ID/governed-financial-advisor:latest` (updated by Path B)
**Timeout**: 5 minutes
...
**Image**: `gcr.io/YOUR_GCP_PROJECT_ID/agentsight-ui:latest` (updated by Path B)
**Timeout**: 5 minutes
...
**Expected output**: `SUCCESS` with image pushed to `us-central1-docker.pkg.dev/YOUR_GCP_PROJECT_ID/governance-stack/lula:0.9.0`.
...
kubectl set image deployment/lula-sc4-watch \
  lula-watch=us-central1-docker.pkg.dev/YOUR_GCP_PROJECT_ID/governance-stack/lula:<known-good-tag> \
  -n governance-stack
```
...
**Risk**: MEDIUM
**Dependency rationale**: Fix after Issue 2 because the CronJob uses `gcr.io/YOUR_GCP_PROJECT_ID/compliance-bridge:latest` — if the compliance-bridge image is stale or missing, the CronJob will continue to fail. The Path B build (Section 3) will push a fresh `compliance-bridge:latest`, which may itself resolve the CronJob failures. Diagnose first; apply targeted fix based on root cause found in Section 1.1.
...
kubectl create secret generic langfuse-compliance-secrets \
  ...
  --from-literal=GOOGLE_CLOUD_PROJECT=YOUR_GCP_PROJECT_ID
```
...
|---|---|---|---|
| `governed-financial-advisor` | `gcr.io/YOUR_GCP_PROJECT_ID/` | ✅ Yes | Inline in script (lines 88–125) |
| `vllm-streamer` | `gcr.io/YOUR_GCP_PROJECT_ID/` | ✅ Yes | [`deployment/docker/cloudbuild.vllm.yaml`](../deployment/docker/cloudbuild.vllm.yaml) |
| `gateway` | `gcr.io/YOUR_GCP_PROJECT_ID/` | ✅ Yes | [`src/gateway/Dockerfile`](../src/gateway/Dockerfile) via `build_image()` |
| `agentsight-ui` | `gcr.io/YOUR_GCP_PROJECT_ID/` | ✅ Yes | [`src/agentsight-ui/Dockerfile`](../src/agentsight-ui/Dockerfile) via `build_image()` |
| `compliance-bridge` | `gcr.io/YOUR_GCP_PROJECT_ID/` | ✅ Yes | [`src/compliance_bridge/Dockerfile`](../src/compliance_bridge/Dockerfile) via `build_image()` |
| `nemo-guardrails` | `gcr.io/YOUR_GCP_PROJECT_ID/` | ✅ Yes | `Dockerfile.nemo` via `build_image()` |
| `lula` | `us-central1-docker.pkg.dev/YOUR_GCP_PROJECT_ID/governance-stack/` | ❌ **NO** | [`deployment/docker/cloudbuild.lula.yaml`](../deployment/docker/cloudbuild.lula.yaml) — **separate step** |
...
for IMAGE in \
  ...
  compliance-bridge \
  nemo-guardrails; do
  echo -n "Checking gcr.io/YOUR_GCP_PROJECT_ID/${IMAGE}:${SHORT_SHA} ... "
  gcloud container images describe \
    "gcr.io/YOUR_GCP_PROJECT_ID/${IMAGE}:${SHORT_SHA}" \
    --project=YOUR_GCP_PROJECT_ID \
