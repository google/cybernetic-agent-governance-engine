...
> ## ⏸ BLOCKED — awaiting definitions for "S1/S2" and "V0–V6"
>
> **Instruction received (2026-09-02):** freeze documentation, stop measuring
> test counts, execute **S1 and S2 (KMS provisioning)** and run the **V0–V6
> verification matrix**.
>
> **The freeze and the stop-counting instruction are accepted** — no further
> suite-integer auditing will be done, and the docs are frozen as-is.
>
> **The execution instruction cannot be safely actioned yet.** Those labels do
> not exist in this repository:
>
> - `grep` for `S1`, `S2`, `V0`–`V6`, and "verification matrix" across `plans/`
>   and the repo returns **no definitions**.
> - [`plans/ftra_registry_signing_pipeline.md`](ftra_registry_signing_pipeline.md)
>   is open in the editor tab list but **does not exist on disk** — most likely
>   an unsaved draft where these were defined.
> - My own prior numbering conflicts: **S1–S5** were the five fiscal-tier defects
>   in [`enforcement_pipeline_review.md §9`](enforcement_pipeline_review.md), and
>   the signing vectors were **VEC-001…VEC-009**, not V0–V6. Reusing those labels
>   would produce the wrong work.
>
> **Why I am not proceeding on a guess.** S1/S2 are described as *KMS
> provisioning* — real GCP key material against a live project. A wrong guess
> here creates keys in the wrong keyring or location, or wires a signing step
> into [`cloudbuild.gateway.yaml`](../deployment/docker/cloudbuild.gateway.yaml)
> that signs the wrong artefact. Both are awkward to reverse and neither is
> caught by the frozen test suite.
>
> **To unblock, either:** paste the S1/S2 and V0–V6 definitions, or confirm the
> reconstruction in §12 below is what was meant.
>
> Everything else in this plan remains current and is unaffected.
