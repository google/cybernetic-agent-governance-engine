...
**File:** [`cbf.py:211`](../src/gateway/governance/safety/cbf_engine.py:211)

### 3.1 Problem statement
`ControlBarrierFunction.__init__()` sets `self._last_seen_epoch: int = 0` ([:211](../src/gateway/governance/safety/cbf_engine.py:211)). `_check_fence_epoch()` ([:275-324](../src/gateway/governance/safety/cbf_engine.py:275)) rejects reads when `current_epoch < self._last_seen_epoch` — but on a freshly spawned instance (new pod, new Cloud Run revision, restart), `_last_seen_epoch=0` means the *first* read after startup can never trigger a regression rejection, no matter how stale the Redis replica actually is, because any `current_epoch >= 0` passes.
...
### 3.3 Fix design
1. In `setup()` ([:213-225](../src/gateway/governance/safety/cbf_engine.py:213)) — already called at instance bootstrap — read the *current* fence epoch from Redis via `_get_fence_epoch()` ([:261-273](../src/gateway/governance/safety/cbf_engine.py:261)) and set `self._last_seen_epoch` to that value (not 0), **provided** the read Redis node can be independently confirmed to be the primary (or a synchronously-replicated replica). This is the minimum viable fix and closes the "epoch=0 always passes" hole, but does not fully solve "which replica did I read from" — see 3.4.
...
### 4.3 Fix design
Extend `validate_evidence_stream_preconditions()` with additional checks, each raising `ConfigurationError` (hard-fail) or logging `logger.warning`/`logger.critical` (soft-fail) depending on severity, gated by the existing `_IS_PRODUCTION`-style pattern already used in `cbf.py` ([:57-60](../src/gateway/governance/safety/cbf_engine.py:57)) and `routing_seal.py` ([:139-162](../src/gateway/governance/routing_seal.py:139)):
