...
# generate_certs.sh — Generate mTLS certificates for provider_02 integration
#
# Creates:
# 1. Root CA certificate (ca.crt) — for Provider 02's trust store
# 2. Client certificate (client.crt) — CAGE's mTLS identity
# 3. Client private key (client.key) — CAGE's signing key
#
# Subject Alternative Name (SAN): cage.governance.provider02.example.com
# Common Name (CN): CAGE Governance Engine - provider_02 Client
...
echo "[INFO] Generating provider_02 mTLS certificates in $CERT_DIR"
...
openssl req -new -x509 \
  ...
  -out ca.crt \
  -days 3650 \
  -subj "/C=US/ST=California/L=Mountain View/O=CAGE Reference Architecture/OU=Governance Integrations/CN=CAGE provider_02 Root CA" \
  2>/dev/null
...
CN = CAGE Governance Engine - provider_02 Client

[req_ext]
subjectAltName = @alt_names

[alt_names]
DNS.1 = cage.governance.provider02.example.com
DNS.2 = cage-provider02.governance.example.com
EOF
...
echo ""
echo "Files created:"
echo "  - ca.crt        → Root CA certificate (send to Provider 02)"
...
echo ""
echo "────────────────────────────────────────────────────────────────────────────────"
echo "Identity Details for Provider 02 Registration:"
echo "────────────────────────────────────────────────────────────────────────────────"
echo "Subject CN: CAGE Governance Engine - provider_02 Client"
echo "Primary SAN: cage.governance.provider02.example.com"
echo "Secondary SAN: cage-provider02.governance.example.com"
echo ""
echo "════════════════════════════════════════════════════════════════════════════════"
