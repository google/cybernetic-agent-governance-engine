...
async def tool_executor_node(state: DataAnalystState):
    ...
    if hasattr(last_message, "tool_calls"):
        for tool_call in last_message.tool_calls:
            ...
            logger.debug("--- [DEBUG] tool_executor checking %s with %s ---", tool_name, tool_args)
            if tool_name in tools_by_name:
                try:
                    logger.debug("--- [DEBUG] tool_executor invoking %s... ---", tool_name)
                ...
                logger.debug("--- [DEBUG] tool_executor WARNING: %s not in tools_by_name ---", tool_name)
                
    logger.debug("--- [DEBUG] tool_executor returning %d tool outputs ---", len(tool_outputs))
...
def analyst_thinker_node(state: DataAnalystState):
    ...
    with tracer.start_as_current_span("DataAnalyst: Thinker") as span:
        ...
        span.set_attribute("langfuse.trace.metadata.current_node", "data_analyst_thinker")
        span.set_attribute("gen_ai.operation.name", "chat")

        _reasoning_base = os.getenv("VLLM_REASONING_API_BASE")
        if not _reasoning_base:
            raise RuntimeError("VLLM_REASONING_API_BASE must be set — no localhost fallback in production")
        llm = ChatOpenAI(
            model=model_name,
        ...
        input_str = "\n".join([m.content for m in messages if isinstance(getattr(m, 'content', None), str)])
        span.set_attribute("gen_ai.input.messages", json.dumps([{"role": "user", "content": input_str}]))
        ...
        span.set_attribute("gen_ai.output.messages", json.dumps([{"role": "assistant", "content": clean_plan}]))
...
async def analyst_doer_node(state: DataAnalystState):
    ...
    with tracer.start_as_current_span("DataAnalyst: Doer") as span:
        ...
        analyst_tool_names = {"get_market_data", "check_market_status", "get_market_sentiment"}
        mcp_tools = [t for t in all_mcp_tools if t.name in analyst_tool_names]
        
        logger.debug("--- [DEBUG] Loaded MCP Tools: %s ---", [t.name for t in mcp_tools])
        ...
        response = await llm_with_tools.ainvoke(messages, config={"run_name": "Analyst Doer"})
        logger.debug("--- [DEBUG] Llama 3.1 Response: %s ---", response)
        
        output_str = response.content if response.content else str(getattr(response, 'tool_calls', 'No content'))
        span.set_attribute("gen_ai.output.messages", json.dumps([{"role": "assistant", "content": output_str}]))
...
def analyst_reporter_node(state: DataAnalystState):
    ...
    with tracer.start_as_current_span("DataAnalyst: Reporter") as span:
        model_name = os.getenv("MODEL_FAST")
        span.set_attribute("langfuse.observation.model.name", model_name)
        span.set_attribute("langfuse.trace.metadata.current_node", "data_analyst_reporter")
        span.set_attribute("gen_ai.operation.name", "chat")
        ...
        span.set_attribute("gen_ai.output.messages", json.dumps([{"role": "assistant", "content": response.content}]))
