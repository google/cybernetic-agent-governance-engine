...
class QueryCache:
    ...
    NON_CACHEABLE_PATTERNS = [
        ...
        r"current price", r"market performance", r"stock price",
        r"today", r"now", r"recent", r"latest",
        
        # User-specific (personalized)
        r"my portfolio", r"should I", r"recommend for me",
        r"I want to", r"help me",
    ...
    async def set(self, query: str, response: str, governance_context: dict | None = None):
        ...
        if self.redis:
            try:
                ...
                logger.info("QueryCache SET (Redis): %s (TTL=%ds)", query[:50], self.ttl)
                return
            except Exception as e:
