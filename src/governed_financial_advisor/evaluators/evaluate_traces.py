...
def main():  # type: ignore[no-untyped-def]
    ...
    for trace_item in traces_data:
        ...
        for obs in obs_response.get("data", []):
            ...
            input_text = (
                json.dumps(obs_input)
                if isinstance(obs_input, (dict, list))
                else str(obs_input)
            )
            output_text = (
                json.dumps(obs_output)
                if isinstance(obs_output, (dict, list))
                else str(obs_output)
            )
            ...
            logger.info(
                f"Evaluating Observation '{obs_name}' (ID: {obs_id}) from Trace {trace_id}..."
            )
            ...
            if eval_result:
                ...
                with _otel_tracer.start_as_current_span(
                    "evaluation.score.financial_compliance"
                ) as span:
                    ...
                    span.set_attribute(
                        ...
                        float(eval_result.get("compliance_score", 0)),
                    )
                    span.set_attribute(
                        "evaluation.score_comment", str(reasoning) if reasoning else ""
                    )

                # Push Helpfulness Score via OTel span event
                with _otel_tracer.start_as_current_span(
                    "evaluation.score.helpfulness"
                ) as span:
                    span.set_attribute("evaluation.trace_id", str(trace_id))
                    span.set_attribute("evaluation.score_name", "helpfulness")
                    span.set_attribute(
                        "evaluation.score_value",
                        float(eval_result.get("helpfulness_score", 1)),
                    )
                    span.set_attribute(
                        "evaluation.score_comment", str(reasoning) if reasoning else ""
                    )
                ...
                logger.info(
                    f"✅ Scored: Compliance={eval_result.get('compliance_score')}, Helpfulness={eval_result.get('helpfulness_score')}"
                )
                count += 1
