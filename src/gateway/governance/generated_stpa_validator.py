# Copyright 2026 Google LLC
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.
#
# =============================================================================
# AUTO-GENERATED by stpa_compiler — DO NOT EDIT
# Source: config/stpa_control_structure.yaml
# Generated: 2026-09-25T21:11:26.283464+00:00
# =============================================================================
# System: CAGE Financial Advisor v1.1.0
#
# Generated Python validator.  Extend GeneratedSTPAValidator in your
# STPAValidator subclass or use it directly as a drop-in replacement.
#
# All threshold references are resolved from the THRESHOLDS singleton
# (config/governance_thresholds.json).

from __future__ import annotations

import logging
import math
from typing import Any

from src.gateway.governance.contracts import Violation, ViolationKind
from src.gateway.governance.schemas.thresholds import THRESHOLDS

logger = logging.getLogger("Gateway.Governance.GeneratedSTPAValidator")


class GeneratedSTPAValidator:
    """
    Auto-generated STPA validator.

    Checks all UCAs with enforcement=[python] or enforcement=[all] that were
    defined in config/stpa_control_structure.yaml.

    Do NOT edit this file; re-run the compiler instead:
        python -m src.gateway.governance.stpa_compiler compile
    """

    def validate(self, action_name: str, params: dict[str, Any]) -> list[Violation]:
        """Public entry-point — delegates to validate_generated().

        Call-sites that previously used STPAValidator.validate() can use this
        method directly on GeneratedSTPAValidator without going through the
        deprecated shim in stpa_validator.py.
        """
        return self.validate_generated(action_name, params)

    def validate_generated(
        self, action_name: str, params: dict[str, Any]
    ) -> list[Violation]:
        """Run all generated UCA checks. Returns list of Violation objects."""
        violations: list[Violation] = []
        _v = self._check_uca_1(action_name, params)
        if _v:
            violations.append(_v)
        _v = self._check_uca_2(action_name, params)
        if _v:
            violations.append(_v)
        _v = self._check_uca_5(action_name, params)
        if _v:
            violations.append(_v)
        _v = self._check_uca_6(action_name, params)
        if _v:
            violations.append(_v)
        _v = self._check_uca_8(action_name, params)
        if _v:
            violations.append(_v)
        _v = self._check_uca_9(action_name, params)
        if _v:
            violations.append(_v)
        _v = self._check_uca_12(action_name, params)
        if _v:
            violations.append(_v)
        return violations

    def _check_uca_1(self, action_name: str, params: dict) -> Violation | None:
        """UCA-1: Agent executes write operation without a signed approval token."""
        try:
            if action_name != "write_db":
                return None
            if params.get("approval_token") is None:
                return Violation(
                    tier="stpa",
                    code="STPA_UCA_UCA_1",
                    message="Agent executes write operation without a signed approval token.",
                    kind=ViolationKind.HARD,
                )
            return None
        except Exception as exc:
            logger.error("Error evaluating UCA-1: %s", exc)
            return Violation(
                tier="stpa",
                code="STPA_UCA_UCA_1",
                message=f"Evaluation error — failing closed ({exc}).",
                kind=ViolationKind.HARD,
            )

    def _check_uca_2(self, action_name: str, params: dict) -> Violation | None:
        """UCA-2: Agent executes trade with stale market data (latency > threshold)."""
        try:
            if action_name != "execute_trade":
                return None
            threshold = THRESHOLDS.stpa.max_latency_ms
            val = params.get("latency_ms")
            if val is None:
                logger.warning("UCA-2: missing param `latency_ms` — failing closed.")
                return Violation(
                    tier="stpa",
                    code="STPA_UCA_UCA_2",
                    message="Missing required param `latency_ms`.",
                    kind=ViolationKind.HARD,
                )
            if float(val) > threshold:
                return Violation(
                    tier="stpa",
                    code="STPA_UCA_UCA_2",
                    message=f"Agent executes trade with stale market data (latency > threshold). ({float(val):.4f} > {threshold})",
                    kind=ViolationKind.HARD,
                )
            return None
        except Exception as exc:
            logger.error("Error evaluating UCA-2: %s", exc)
            return Violation(
                tier="stpa",
                code="STPA_UCA_UCA_2",
                message=f"Evaluation error — failing closed ({exc}).",
                kind=ViolationKind.HARD,
            )

    def _check_uca_5(self, action_name: str, params: dict) -> Violation | None:
        """UCA-5: Agent executes buy order when daily drawdown exceeds limit."""
        try:
            if action_name != "execute_trade":
                return None
            threshold = THRESHOLDS.stpa.uca5_drawdown_threshold_pct
            val = params.get("drawdown")
            if val is None:
                logger.warning("UCA-5: missing param `drawdown` — failing closed.")
                return Violation(
                    tier="stpa",
                    code="STPA_UCA_UCA_5",
                    message="Missing required param `drawdown`.",
                    kind=ViolationKind.HARD,
                )
            if float(val) > threshold:
                return Violation(
                    tier="stpa",
                    code="STPA_UCA_UCA_5",
                    message=f"Agent executes buy order when daily drawdown exceeds limit. ({float(val):.4f} > {threshold})",
                    kind=ViolationKind.HARD,
                )
            return None
        except Exception as exc:
            logger.error("Error evaluating UCA-5: %s", exc)
            return Violation(
                tier="stpa",
                code="STPA_UCA_UCA_5",
                message=f"Evaluation error — failing closed ({exc}).",
                kind=ViolationKind.HARD,
            )

    def _check_uca_6(self, action_name: str, params: dict) -> Violation | None:
        """UCA-6: Agent submits market order exceeding allowable fraction of daily volume."""
        try:
            if action_name != "execute_trade":
                return None
            # Composite condition: order_size > threshold_ref(stpa.uca6_max_order_volume_fraction) * daily_vol
            # Implement custom logic here.
            pass
            return None
        except Exception as exc:
            logger.error("Error evaluating UCA-6: %s", exc)
            return Violation(
                tier="stpa",
                code="STPA_UCA_UCA_6",
                message=f"Evaluation error — failing closed ({exc}).",
                kind=ViolationKind.HARD,
            )

    def _check_uca_8(self, action_name: str, params: dict) -> Violation | None:
        """UCA-8: Agent executes trade before risk assessment is complete."""
        try:
            if action_name != "execute_trade":
                return None
            if params.get("risk_assessed") is False:
                return Violation(
                    tier="stpa",
                    code="STPA_UCA_UCA_8",
                    message="Agent executes trade before risk assessment is complete.",
                    kind=ViolationKind.HARD,
                )
            return None
        except Exception as exc:
            logger.error("Error evaluating UCA-8: %s", exc)
            return Violation(
                tier="stpa",
                code="STPA_UCA_UCA_8",
                message=f"Evaluation error — failing closed ({exc}).",
                kind=ViolationKind.HARD,
            )

    def _check_uca_9(self, action_name: str, params: dict) -> Violation | None:
        """UCA-9: Agent executes trade with compliance check bypassed."""
        try:
            if action_name != "execute_trade":
                return None
            if params.get("compliance_checked") is False:
                return Violation(
                    tier="stpa",
                    code="STPA_UCA_UCA_9",
                    message="Agent executes trade with compliance check bypassed.",
                    kind=ViolationKind.HARD,
                )
            return None
        except Exception as exc:
            logger.error("Error evaluating UCA-9: %s", exc)
            return Violation(
                tier="stpa",
                code="STPA_UCA_UCA_9",
                message=f"Evaluation error — failing closed ({exc}).",
                kind=ViolationKind.HARD,
            )

    def _check_uca_12(self, action_name: str, params: dict) -> Violation | None:
        """UCA-12: Agent executes bounded trade when bounding contracts B1-B10 fail validation. Phase 5: execute_trade_bounded is EXTERNALLY_REVERSIBLE when B10 rollback window validation passes, enabling HITL review without confidence hard-gate per VEC-004. When B10 fails, classification override to IRREVERSIBLE_TERMINAL prevents execution."""
        try:
            if action_name != "execute_trade_bounded":
                return None
            return None
        except Exception as exc:
            logger.error("Error evaluating UCA-12: %s", exc)
            return Violation(
                tier="stpa",
                code="STPA_UCA_UCA_12",
                message=f"Evaluation error — failing closed ({exc}).",
                kind=ViolationKind.HARD,
            )
