...
def install_domain_components(
    ...
    *,
    ...
    Args:
        governor: SymbolicGovernor instance to install components into
    ...
    if consensus_engine_impl is not None:
        if not isinstance(governor.consensus_engine, NullConsensusProvider):
            raise RuntimeError("consensus_engine already installed by another plugin")
        governor.consensus_engine = consensus_engine_impl
        logger.info(
