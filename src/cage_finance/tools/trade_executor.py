...
_USE_MOCK_BROKER: bool = os.getenv("USE_MOCK_BROKER", "false").lower() in (
    "true",
    "1",
    "yes",
)
...
async def execute_trade(order: TradeOrder, *, routing_seal: str) -> str:
    """
    ...
    logger.info(
        f"Executing Trade {order.transaction_id} on {base_url} (Confidence: {order.confidence})..."
    )
    ...
    try:
        ...
        if latest_violation:
            raise RuntimeError(
                f"Trade INTERRUPTED immediately before HTTP call: {latest_violation}"
            )
        ...
        return (
            f"EXECUTED: {order.symbol} x {order.amount} (Order ID: {result.get('id')})"
        )
