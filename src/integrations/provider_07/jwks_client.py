...
"""
JWKS Client for Out-of-Band Ed25519 Key Resolution.

Implements key resolution from a remote JWKS manifest with in-memory caching
and TTL-based expiration. Enforces the Trust Anchor Invariant: never parse keys
supplied inside signed message payloads.

Key Resolution Flow:
1. Check in-memory cache against TTL
2. On cache miss/expiry, fetch JWKS manifest via HTTP
3. Parse Ed25519 keys matching kty="OKP", crv="Ed25519"
4. Return None if kid is unknown (caller must fail closed)

Security Invariants:
- URL scheme validation (http/https only) for Bandit B310 compliance
...
logger = logging.getLogger("cage.provider_07.jwks_client")


class Provider07JwksClient:
    """
    Out-of-band JWKS client for Ed25519 public key resolution.

    Fetches and caches Ed25519 public keys from a remote JWKS endpoint.
    Keys are resolved by kid (Key Identifier) and cached in-memory with TTL.

    Attributes:
        jwks_url: URL of the JWKS manifest endpoint
    ...
    def __init__(
        ...
        Args:
            jwks_url: JWKS manifest endpoint (must be http:// or https://)
            cache_ttl_seconds: Cache TTL in seconds (default: 1 hour)
            timeout_seconds: HTTP request timeout (default: 5 seconds)

        Raises:
            ValueError: If jwks_url scheme is not http or https (Bandit B310 safe)
        """
        ...
        if parsed.scheme not in ("http", "https"):
            raise ValueError(
                ...
                f"Only 'http' and 'https' are allowed (Bandit B310 compliance)."
            )
    ...
    async def get_key(self, kid: str) -> Ed25519PublicKey | None:
        ...
        the JWKS manifest from the remote endpoint and parses Ed25519 keys.
    ...
    async def _refresh_keys(self) -> None:
        ...
        """
        logger.info(
            "provider_07: Refreshing JWKS from %s (timeout=%.1fs)",
            self._jwks_url,
        ...
        for jwk in jwks_data["keys"]:
            ...
            if not kid or not x_b64:
                logger.warning(
                    "provider_07: Skipping JWK with missing kid or x: %s", jwk
                )
                continue
            ...
            try:
                ...
                logger.debug("provider_07: Loaded Ed25519 key kid=%s", kid)
            except Exception as exc:
                logger.warning(
                    "provider_07: Failed to parse Ed25519 key kid=%s: %s", kid, exc
                )
                continue
        ...
        logger.info(
            "provider_07: JWKS cache updated with %d keys (ttl=%ds)",
            len(new_cache),
