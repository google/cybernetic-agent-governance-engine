...
def _flush_with_timeout(
    lf_client: Any, timeout: float = _LANGFUSE_SDK_TIMEOUT_S
) -> None:
...
def _ingest_sync(
        ...
        try:
            trace = compliance_langfuse.trace(
                ...
                metadata={
                    ...
                    "safety_rate": str(finding.safety_rate)
    ...
    logger.info(
        "[audit_workflow] Flushed %d traces and scores to Langfuse compliance project.",
        len(findings),
    )
    return len(findings)
...
async def _step4_alert_on_critical_fail(
    ...
    logger.info(
        "[audit_workflow] Critical alert sent for: %s", ", ".join(alert_targets)
    )
    ...
    def _sync() -> list[dict]:
        ...
        try:
            ...
            for t in response.data or []:
                ...
                outcome = (
                    metadata.get("iso_42001_outcome")
...
async def _step5_one_control(
    ...
    try:
        ...
        logger.warning(
            "[audit_workflow] Failed to publish REMEDIATION_GENERATED (non-fatal): %s",
            exc,
        )
...
async def _step5_remediation_advisor(
    ...
    if not alerted or not alert_targets:
        logger.info(
            "[audit_workflow] No critical alert — skipping remediation advisory."
        )
        return {
            "remediation_sent": False,
    ...
    if not vllm_base:
        logger.warning(
            "[audit_workflow] VLLM_BASE_URL not set — skipping LLM advisory."
        )
        return {
            "remediation_sent": False,
            "remediation_text": None,
    ...
    first_err = next(
        (r.get("advisor_error") for r in per_control if r.get("advisor_error")), None
    )
...
async def run_audit_workflow(oscal_yaml: str, audit_id: str) -> dict:
    ...
    try:
        ...
        logger.warning(
            "[audit_workflow] Failed to publish CONTEXT_CHAIN_SEALED (non-fatal): %s",
            exc,
        )
    ...
    try:
        ...
        logger.warning(
            "[audit_workflow] Failed to publish AUDIT_FINDING events (non-fatal): %s",
            exc,
        )
    ...
    if alerted and alert_targets:
        try:
            for control_id in alert_targets:
                failing_finding = next(
                    (
                        f
                        for f in findings
                        if f.control_id == control_id and f.result == "FAIL"
                    ),
                    None,
                )
            ...
            logger.warning(
                "[audit_workflow] Failed to publish GOVERNANCE_VIOLATION events (non-fatal): %s",
                exc,
            )
    ...
    async def _step5_bg() -> None:
        ...
        try:
            ...
            logger.warning(
                "[audit_workflow] Step 5 (remediation advisor) failed (non-fatal): %s",
                exc,
            )
    ...
    try:
        ...
        if os.environ.get("OSCAL_S3_ENDPOINT"):
            try:
                ...
                logger.info(
                    "[audit_workflow] AARM conformance report persisted: %s",
                    persisted_key,
                )
                aarm_report_artifact = persisted_key
            except Exception as exc:
                logger.warning(
                    "[audit_workflow] AARM report persistence failed (non-fatal): %s",
                    exc,
                )
        logger.info(
        ...
        logger.warning(
            "[audit_workflow] Step 6 (AARM conformance report) failed (non-fatal): %s",
            exc,
        )
