...
class AgentBenchmark:
    ...
    def run_inference(self, prompt: str):
        ...
        try:
            ...
            )  # Use self.endpoint and original timeout
...
def run_benchmark(benchmark, prompt, iterations):
    ...
    for i in range(iterations):
        ...
        if stats:
            ...
            print(
                f" {stats['latency']:.3f}s | ~{int(stats['completion_tokens'])} tok | {stats['throughput']:.1f} tok/s"
            )
        else:
            print(" Failed")
...
def main():
    ...
    parser.add_argument(
        "--prompt",
        default="Analyze the stock performance of AAPL.",
        help="Prompt to use",
    )
