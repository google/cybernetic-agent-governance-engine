...
def _get_live_credentials() -> tuple[str, str]:
    ...
    endpoint = os.environ.get("PROVIDER_03_ENDPOINT", "").split("#")[0].strip()

    api_key = os.environ.get("PROVIDER_03_API_KEY", "").split("#")[0].strip()
...
async def test_live_validate_fria(
    ...
    if result.admitted:
        assert not any(f.get("severity") == "blocked" for f in result.findings), (
