...
class TestIsoControlMapSingleSource:
    ...
    def test_langfuse_utils_imports_from_types(self):
        ...
        from src.governed_financial_advisor.utils.langfuse_utils import ISO_CONTROL_MAP as lf_map
...
class TestAuditStatusEndpoint:
    ...
    def test_ingest_then_status_returns_done(self, client):
        mock_result = {
            "status": "ok", "audit_id": "status-test-001",
            "findings_count": 1, "artifact_key": None,
            "ingested": 1, "alerted": False, "alert_targets": [],
            "remediation_sent": False, "remediation_text": None,
            "advisor_error": None,
        }
        with patch(
    ...
    def test_status_store_lru_eviction(self):
        ...
        from src.compliance_bridge.main import _set_audit_status, _audit_status, _AUDIT_STATUS_MAXSIZE
        _audit_status.clear()
...
class TestCageRoutingSealGuard:
    ...
    def test_production_with_strong_secret_ok(self):
        assert self._attempt_startup("production", "hunter2-very-long-random-secret") is True
