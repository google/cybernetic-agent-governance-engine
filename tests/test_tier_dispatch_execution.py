...
class TestTierDispatchOrdering:
    ...
    @pytest.mark.asyncio
    async def test_unclaimed_tiers_do_not_execute(
        self, mock_governor: SymbolicGovernor
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_violations_fail_fast_on_first_tier(
        self, mock_governor: SymbolicGovernor
    ) -> None:
        """First violation stops execution (fail-fast behavior)."""
        gov = mock_governor
        ...
        # Fail-fast: only first tier's violation is returned
        assert len(violations) == 1
        assert violations[0].tier == "tier1"
        assert violations[0].code == "RULE_A"
        # Second tier did not execute (fail-fast)
        executed_tiers = [name for name, _ in OrderTrackingTier.execution_log]
        assert executed_tiers == ["tier1"]
