...
def _get_live_config() -> tuple[str, str]:
    ...
    registry_url = (
        os.environ.get("PROVIDER_05_BLUEPRINT_REGISTRY_URL", "")
        .split("#")[0]
        .strip()
    )

    api_key = (
        os.environ.get("PROVIDER_05_API_KEY", "")
        .split("#")[0]
        .strip()
    )
...
@pytest.fixture
def live_blueprint_provider(live_client: Provider05Client) -> Provider05BlueprintProvider:
