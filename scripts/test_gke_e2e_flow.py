...
def test_gateway_governance_flow():
    ...
    try:
        response = httpx.post(
            f"{gateway_url}/mcp", json={"method": "tools/list"}, timeout=10
        )
    ...
    try:
        response = httpx.get("http://localhost:3002/v1/controls", timeout=10)
        if response.status_code == 200:
    ...
    try:
        ...
        response = httpx.post(
            ...
            json={
                "id": trace_id,
                "name": "smoke-test",
                "metadata": {"test": "gke-smoke-test", "timestamp": int(time.time())},
            },
            timeout=10,
        )
