...
ARCHITECTURE_SPEC = PROJECT_ROOT / "docs" / "architecture" / "provider_02_native_schema_spec.md"
...
def create_archives(files: list[Path], manifest: dict[str, Any], project_root: Path, dist_dir: Path, basename: str) -> tuple[Path, Path]:
...
def print_summary(files: list[Path], manifest: dict[str, Any], tar_path: Path, zip_path: Path) -> None:
...
def main() -> int:
    ...
    print()
    print("╔" + "=" * 78 + "╗")
    print("║" + " " * 15 + "Provider_02 Native Schema Handoff Packager" + " " * 21 + "║")
    print("╚" + "=" * 78 + "╝")
    print()
    ...
    tar_path, zip_path = create_archives(files, manifest, PROJECT_ROOT, DIST_DIR, ARCHIVE_BASENAME)
