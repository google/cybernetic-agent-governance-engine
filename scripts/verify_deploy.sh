#!/usr/bin/env bash
# verify_deploy.sh — Post-deploy verification for CAGE GKE deployments.
#
# Checks:
#   1. Core deployment image tags vs. latest Cloud Build results
#   2. reconciliation-worker-secrets has gcs-reconciliation-bucket key
#   3. reconciliation-worker CronJob has at least one successful Job run
#
# Usage:
#   ./scripts/verify_deploy.sh [--namespace <ns>] [--project <project>]
#
# Exits 0 only if no ❌ checks fail.
