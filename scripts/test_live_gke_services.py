...
def main():
    ...
    results["langfuse_health"] = test_http_service("Langfuse Health", "http://localhost:3001/api/public/health")
    results["gateway"] = test_http_service("Gateway", "http://localhost:8080/health")
    results["backend"] = test_http_service("Backend/GFA", "http://localhost:8081/health")
    results["compliance"] = test_http_service("Compliance Bridge", "http://localhost:3002/health")
    results["vllm_fast"] = test_http_service("vLLM Fast", "http://localhost:8001/v1/models", timeout=10)
    results["vllm_reasoning"] = test_http_service("vLLM Reasoning", "http://localhost:8000/v1/models", timeout=10)
