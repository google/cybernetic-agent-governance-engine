...
def main():
    ...
    if cage_region:
        print(f"ℹ️  CAGE_DEPLOYMENT_REGION={cage_region} — jurisdiction-specific checks active")
    else:
        print("ℹ️  CAGE_DEPLOYMENT_REGION not set — running universal ISO 42001 checks only")
    print()
    success = True

    # 1. Unit Tests (Universal — ISO 42001 A.9.2 continuous monitoring)
    if not run_command("pytest tests/ --maxfail=1 --disable-warnings -q", "Unit Tests (Pytest — Universal ISO 42001)"):
        success = False
    ...
    if not run_command("opa test tests/opa/ -v", "OPA Policy Checks (Universal ISO 42001)"):
    ...
    if cage_region == "US_FED":
        ...
        if not run_command("opa test tests/opa/us_fed/ -v", "OPA Policy Checks (US_FED — NIST SP 800-53)"):
    ...
    # 2b. EU_ECB-only OPA checks (EU AI Act / GDPR / DORA)
        ...
        if not run_command("opa test tests/opa/eu_ecb/ -v", "OPA Policy Checks (EU_ECB — EU AI Act)"):
    ...
    # 2c. APAC_MAS-only OPA checks (MAS FEAT / MAS Notice 655 / MAS TRM)
        ...
        if not run_command("opa test tests/opa/apac_mas/ -v", "OPA Policy Checks (APAC_MAS — MAS FEAT)"):
