...
def fetch_data():
    try:
        ...
        res = requests.get(
            url,
            auth=auth,
            params={"type": "GENERATION", "public": "false", "limit": 100},
        )
        ...
        for obs in obs_data:
            ...
            if start and comp:
                ...
                print(
                    f"Generation [Trace: {trace_id}] -> TTFT: {ttft_ms:.2f}ms | Latency: {obs.get('latency')}s"
                )
        ...
        res = requests.get(
            url, auth=auth, params={"type": "SPAN", "public": "false", "limit": 100}
        )
        res.raise_for_status()
        ...
        for span in span_data[:20]:
            ...
            if (
                "injection" in name
                or "identity-tag" in name
                or "financial-advisor-sa" in name
            ):
