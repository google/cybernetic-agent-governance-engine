...
>
> **Cluster context assumed:** `gke_laah-cybernetics_us-central1-a_cage-dev`, namespace `governance-stack`. Adjust for the target cluster before running any `kubectl` command.
...
- [ ] Trivy scan passes — no unmitigated CRITICAL CVEs:
  ...
  ```bash
  trivy image gcr.io/laah-cybernetics/cage-gateway:latest \
