...
**Change summary:** A new helper method routes violations to the appropriate governance
...
### G3: FTRA Boundary Check Now Mandatory

**Change summary:** The FTRA (Finite-Time Reachability Analysis) boundary check is no
longer gated by an environment flag; it runs unconditionally on every governance
evaluation.

**Previous behavior:**
```python
if os.getenv("CAGE_FTRA_BOUNDARY_ENABLED", "false").lower() == "true":
    self._ftra_boundary_check(...)
```

**Current behavior:**
```python
self._ftra_boundary_check(...)  # Always runs
```
...
**Paper impact:** §4.7 (zero-trust controls) must note that FTRA boundary checking is
now mandatory, not opt-in.

### G4: Reconciliation Replay Defense Now Implemented

**Change summary:** The replay attack vulnerability described in §7.2/§7.3 as an "open,
unmitigated vulnerability" has been implemented as an opt-in defense mechanism.

**Implementation:**
- Environment variable: `CAGE_RECONCILIATION_REPLAY_DEFENSE`
- When enabled, the reconciliation worker validates that ledger entries are not replayed
...
implemented (opt-in) control, not an open vulnerability. The limitation should be
reframed as "opt-in status" rather than "unmitigated."

### G5: Evidence Chain Blocking Gate Default Changed

**Change summary:** The `EVIDENCE_CHAIN_BLOCKING` environment variable now defaults to
`"true"` instead of `"false"`.

**Implication:** Evidence chain writes are now blocking by default, ensuring that
compliance evidence is durably recorded before the governance decision is returned.
