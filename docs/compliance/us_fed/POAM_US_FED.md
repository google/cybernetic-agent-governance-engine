...
see_also:
  - compliance/universal/POAM_ISO42001.md   # universal ISO 42001 AIMS weaknesses (all regions)
  ...
  - docs/compliance/us_fed/NIST_AI_600_1_US_FED_ANALYSIS.md  # AI 600-1 gap analysis (source for AI600-xxx items)
---
...
> **⚠️ Scope Notice:** This document tracks weaknesses against the **NIST SP 800-53 Rev. 5** control baseline, the **NIST RMF authorization process**, and **NIST AI 600-1** (Generative AI Profile). It applies exclusively to `US_FED` deployments. For universal ISO 42001 AIMS weaknesses (all regions), see [`compliance/universal/POAM_ISO42001.md`](../universal/POAM_ISO42001.md). For EU_ECB-specific weaknesses, see [`compliance/eu_ecb/POAM_EU_ECB.md`](../eu_ecb/POAM_EU_ECB.md). For APAC_MAS-specific weaknesses, see [`compliance/apac_mas/POAM_APAC_MAS.md`](../apac_mas/POAM_APAC_MAS.md). For the full AI 600-1 gap analysis, see [`docs/compliance/us_fed/NIST_AI_600_1_US_FED_ANALYSIS.md`](NIST_AI_600_1_US_FED_ANALYSIS.md).
...
> **Note:** POAM-018 and POAM-019 are also tracked in [`compliance/universal/POAM_ISO42001.md`](../universal/POAM_ISO42001.md) as universal ISO 42001 §A.9.4 weaknesses (all regions). The entries below are retained here for NIST SP 800-53 AU-9 / SC-7 audit continuity.
>
> **v2.1 addition (2026-06-15):** Seven new POAM items (AI600-001 through AI600-007) have been added based on the NIST AI 600-1 gap analysis in [`docs/compliance/us_fed/NIST_AI_600_1_US_FED_ANALYSIS.md`](NIST_AI_600_1_US_FED_ANALYSIS.md). These items address GenAI-specific risks required for US_FED ATO under EO 14110 and OMB M-24-10.
>
> **v2.1 update (2026-06-15):** Phase 0–3 implementation work completed on branch `feat/CAGE-AI600-ai600-1-implementation` (commits `c014337`–`51f6c25`). AI600-001, AI600-002, AI600-003, AI600-005, and AI600-007 moved to **In Progress**. AI600-004 and AI600-006 remain **Open** (no source code changes yet). 222 AI 600-1 unit tests pass (0 failures). See [`docs/compliance/us_fed/AI_600_1_IMPLEMENTATION_PLAN.md`](AI_600_1_IMPLEMENTATION_PLAN.md) for the full implementation roadmap.
...
| POAM-018 | AU-9 | §A.9.4 | ALL | ~~Langfuse compliance project credentials fail silently when absent.~~ **✅ Closed (verified 2026-09-22).** Both remediation clauses are implemented at HEAD: (1) `_validate_langfuse_credentials()` raises `RuntimeError` in non-development environments ([`audit_workflow.py:156`](../../../src/compliance_bridge/audit_workflow.py), commit `489949a`); (2) `GET /health` reports `langfuse_compliance_configured` ([`main.py:381,421`](../../../src/compliance_bridge/main.py), commit `fd5e28b`), asserted by [`tests/test_gateway_compliance_bridge_contract.py:164,197`](../../../tests/test_gateway_compliance_bridge_contract.py). Phase 4 validation (2026-06-05) had already confirmed live credentials in-cluster. | Code review + test verification 2026-09-22 | **High** | 2026-07-15 | 4 hrs | **Closed** | Both clauses shipped; closure verified 2026-09-22 against commits `489949a` and `fd5e28b`. **See also:** [`compliance/universal/POAM_ISO42001.md#POAM-018`](../universal/POAM_ISO42001.md) — tracked as ISO 42001 §A.9.4 universal weakness. |
| POAM-019 | AU-9, SC-7 | §A.9.4 | ALL | ~~Terraform fallback silently collapses dual-project telemetry isolation.~~ **✅ Closed (verified 2026-09-22).** The fallback was removed and replaced with fail-closed validation in commit `fd5e28b`: a `lifecycle.precondition` on the `app_secrets` module call ([`main.tf:730`](../../../infra/targets/gcp-gke/main.tf)) rejects an apply when either compliance key is empty **or** equal to its application-project counterpart; [`variables.tf:495-519`](../../../infra/targets/gcp-gke/variables.tf) declares both variables `nullable = false` with validation blocks. | Terraform config review 2026-09-22 | **High** | 2026-07-15 | 2 hrs | **Closed** | Fallback removed; fail-closed precondition + `nullable = false` in place. Closure verified 2026-09-22 against commit `fd5e28b`. **See also:** [`compliance/universal/POAM_ISO42001.md#POAM-019`](../universal/POAM_ISO42001.md) — tracked as ISO 42001 §A.9.4 universal weakness. |
| POAM-020 | CM-3 | §10.2 | US_FED | ~~Technical report README version mismatch.~~ **RESOLVED (LEGACY).** [HISTORICAL ARTIFACT] `docs/technical-report/README.md` references were corrected from v2.1.0 to v0.1.0 prior to documentation consolidation into `docs/architecture/`. | Documentation audit | **Moderate** | 2026-06-15 | 2 hrs | **Closed** | ✅ [HISTORICAL / LEGACY] Fixed in legacy `docs/technical-report/README.md` — directory subsequently consolidated into `docs/architecture/`. Closed 2026-05-27. |
...
| POAM-022 | SA-9, CA-7 | §A.6.1 | US_FED | External Normative Provider interface implemented but operating in stub mode. `normative_provider.py` provides the full 3-endpoint integration surface with adaptive gating primitive (`enforce_fria_boundary()`), but production activation requires external provider API credentials. `CAGE_NORMATIVE_PROVIDER=static` (default) means no external validation is performed — all FRIA checks are stub-admitted. The interface code, daemon lifecycle, DEFER queue integration, and 29 tests are verified, but the control is not providing independent compliance ground truth until credentials are provisioned. | Code review | **Moderate** | 2026-08-31 | 4 hrs | **In Progress** | Coordinate with external provider team for API key provisioning; configure `CAGE_NORMATIVE_ENDPOINT` and `CAGE_NORMATIVE_API_KEY_SECRET` in `prod.tfvars`; verify boot-time baseline fetch and adaptive gating in staging environment. **EU AI Act Art. 29a aspect tracked separately:** [`compliance/eu_ecb/POAM_EU_ECB.md#EU-001`](../eu_ecb/POAM_EU_ECB.md). |
...
> **Source:** [`docs/compliance/us_fed/NIST_AI_600_1_US_FED_ANALYSIS.md`](NIST_AI_600_1_US_FED_ANALYSIS.md) — full gap analysis, recommended RMF chunk updates, OSCAL artifacts, and implementation roadmap.
>
...
_This document is a living record and must be updated within 5 business days of any status change. Unauthorized modification of closed or in-progress items requires ISSO concurrence. This file supersedes `docs/POAM.md` (v1.7, 2026-06-08), POAM_US_FED v2.0 (2026-06-08), and POAM_US_FED v2.1 (2026-06-15). For the cross-region traceability matrix, see [`compliance/cross-region/POAM_INDEX.md`](../cross-region/POAM_INDEX.md). For the AI 600-1 gap analysis that generated AI600-xxx items, see [`docs/compliance/us_fed/NIST_AI_600_1_US_FED_ANALYSIS.md`](NIST_AI_600_1_US_FED_ANALYSIS.md). For the full AI 600-1 implementation plan and phase roadmap, see [`docs/compliance/us_fed/AI_600_1_IMPLEMENTATION_PLAN.md`](AI_600_1_IMPLEMENTATION_PLAN.md)._
