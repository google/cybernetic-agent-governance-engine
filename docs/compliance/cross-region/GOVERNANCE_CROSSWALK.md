...
| :----------------------------------------- | :---------------------------- | :----------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :----------------------------------------------------------------------------------------------------------------------- | :-------------------------------------------- | :-------------------- |
| **ISO A.5.2**: Social impact assessment.   | Map 1.1: Contextual analysis. | **SC-1**: Agent must not output content classified as 'Hate Speech' or 'Bias'. | **NeMo Output Rail** + **Aho-Corasick Tier-1** in [`safety.py`](../src/gateway/governance/safety.py)                                                                                                                                          | [`lula-validation-a52.yaml`](../compliance/lula/lula-validation-a52.yaml) — `safety_rate ≥ 99%`                          | OSCAL Finding, Langfuse compliance project    | **All regions** |
...
- **compliance-bridge** ([`src/compliance_bridge/`](../src/compliance_bridge/)) — Python FastAPI service that aggregates Langfuse traces into a time-windowed `safety_rate` per ISO 42001 control, served via `/v1/metrics/{control_id}`.
