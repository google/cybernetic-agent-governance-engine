...
| POAM-023 | §A.4 — Resource Management / §8 — Operation | NIST SP 800-53 SA-9; ISO 42001 §A.6 | ALL | `AnchorageGrpcLedgerProvider` not yet implemented — FUTURE STATE. The `ControlBarrierFunction` (Tier 2 of the 7-tier SymbolicGovernor) is documented as being externally reconciled via `AnchorageGrpcLedgerProvider` for authoritative cash-balance ledger state. This gRPC-based external ledger provider is **not yet implemented**. The CBF currently relies solely on Redis-backed optimistic locking (WATCH/MULTI/EXEC) for state management. Until the external ledger provider is implemented, the CBF cannot perform authoritative cross-system reconciliation, which is required for ISO 42001 §A.4 resource management completeness and §8 operational integrity. | Architecture review — `src/gateway/governance/cbf.py`, `src/gateway/governance/normative_provider.py` | **Moderate** | 2026-09-08 | 40 hrs | Open | Implement `AnchorageGrpcLedgerProvider` gRPC client in `src/gateway/governance/`; integrate with `ControlBarrierFunction` for external ledger reconciliation; add integration tests; update OSCAL component definition with new control implementation evidence. |
| POAM-024 | §8.4 — Operational Planning and Control | NIST SP 800-53 CA-2; ISO 42001 §9.1 | ALL | Staging environment not provisioned — dev→prod promotion path lacks pre-production validation tier. The Terraform schema (`infra/targets/gcp-gke/variables.tf`, `infra/targets/agnostic/variables.tf`, `infra/modules/k8s_namespace/variables.tf`) formally accepts `staging` as a valid `environment` value, and the change management process (§3.8) defines a three-tier promotion path (`dev → staging → prod`). However, no `staging.tfvars` file exists, no staging cluster has been provisioned, and `deploy_all.sh` rejects `--env staging` with an explicit error. Until staging is provisioned, all changes are promoted directly from `dev` to `prod` with AO acknowledgement. This creates a gap in pre-production validation coverage relative to the intended AIMS operational planning posture. | Architecture review — `infra/targets/gcp-gke/variables.tf`, `deploy_all.sh`, `docs/CHANGE_MANAGEMENT_PROCESS.md §3.8` | **Low** | 2026-12-31 | 16 hrs | Open | Provision `infra/targets/gcp-gke/staging.tfvars` with production-equivalent security settings (`enable_nist_compliance=true`); add `staging` branch to `deploy_all.sh`; update `src/gateway/governance/constants.py` to treat `CAGE_ENV=staging` as prod-equivalent; run full release gate suite against staging before promoting to prod. AO-acknowledged for v2.0.0 release. |
