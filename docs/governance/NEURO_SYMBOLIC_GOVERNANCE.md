# Neuro-Symbolic Governance Architecture — v3.0.1
...
- **Checks (7-Tier Pipeline):**
  ...
  | **2/4** | CBF + OPA concurrent evaluation | `asyncio.gather(cbf_check, opa_check)` | **Tier 2:** `ControlBarrierFunction.atomic_verify_and_commit()` (`cbf.py`) — Redis-backed Lua atomic cash balance invariant `h(x) = cash_balance − min_cash_balance ≥ 0` (γ=0.5, min=$1,000). `atomic_verify_and_commit()` collapses check+commit into a single Lua hop, followed by synchronous `_sync_to_replicas()` via `WAIT`. On replica lag timeout in production (`CAGE_STRICT_REPLICATION=true`), automatic local rollback (`rollback_state()`) fails closed. Monotonic `safety:fence_epoch` counter prevents failover balance replays. `_read_cbf_state_atomic()` prioritises `reconciliation:verified_balance` (KMS-signed external balance from reconciliation worker, POAM-023) then falls back to `safety:current_cash` with CRITICAL audit log. **Tier 4:** `OPAClient.evaluate_policy()` evaluates `trade_governance.rego`. Combined latency = `max(CBF_ms, OPA_ms)`. *(All Regions)* |
...
- **v3.0.1:** The deprecated `stpa_validator.py` shim was removed. Import `GeneratedSTPAValidator` directly from `generated_stpa_validator.py`.
...
| **Symbolic** | OPA Rego (`src/governed_financial_advisor/governance/policy/trade_governance.rego`, `config/opa/`), STPA validator (`generated_stpa_validator.py`), CBF (`cbf.py`) | Formal invariants; mathematical safety properties; fail-closed. **v3.0.1:** The deprecated `safety.py` shim was removed — import directly from `cbf.py`. |
