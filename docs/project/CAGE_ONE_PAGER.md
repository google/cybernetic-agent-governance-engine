...
> **Current posture (2026-09-02):** this paragraph previously read *"for all trades exceeding $10k or risk score > 0.7"*, implying autonomous execution below those thresholds. In the deployed system the FTRA pre-pipeline gate classifies `execute_trade` as `IRREVERSIBLE_TERMINAL` and escalates **every** trade to human approval, so the graduated thresholds are configured but unreachable for this action. Bounded autonomy below a ceiling is planned — see [`plans/bounded_trade_autonomy_plan.md`](../../plans/bounded_trade_autonomy_plan.md) and [`docs/governance/TIER_APPLICABILITY.md`](../governance/TIER_APPLICABILITY.md).
