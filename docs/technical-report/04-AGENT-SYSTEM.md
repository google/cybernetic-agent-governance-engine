...
CAGE governance checks are not advisory: they gate every state transition before execution can reach sensitive nodes (e.g., `governed_trader`). The pipeline is structured so that thinker/doer reasoning, data acquisition, plan generation, evaluation, and safety validation must all complete successfully before an action is executed, with human approval required as the final gate. This architecture ensures no path exists from user instruction to execution that bypasses CAGE policy enforcement.
