...
| --------------------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| Gateway                     | `deployment/docker/cloudbuild.gateway.yaml`                                               | `gcr.io/YOUR_GCP_PROJECT_ID/gateway:latest` — Hybrid Gateway image           |
| Governed Financial Advisor  | `deployment/docker/cloudbuild.advisor.yaml`                                               | `gcr.io/YOUR_GCP_PROJECT_ID/governed-financial-advisor:latest` — Advisor image |
...
**Build flow**: Cloud Build trigger → Docker build → push to GCR (`gcr.io/YOUR_GCP_PROJECT_ID/`) → update Kubernetes Deployment via `kubectl rollout`.

**2026-06-03 deployment:** Both application images were built via Cloud Build and deployed to GKE cluster `gke_YOUR_GCP_PROJECT_ID_us-central1-a_cage-dev`, namespace `governance-stack`. No Terraform/infrastructure files were modified. All pods verified running and healthy.

| Image                                                    | Digest (sha256)                                                    |
| -------------------------------------------------------- | ------------------------------------------------------------------ |
| `gcr.io/YOUR_GCP_PROJECT_ID/gateway:latest`                 | `sha256:f0e8f4170e90b37cb5809eff7de93d63a0c75a57c6a03f1ce6815421c4b8dc65` |
| `gcr.io/YOUR_GCP_PROJECT_ID/governed-financial-advisor:latest` | `sha256:f0e8f4170e90b37cb5809eff7de93d63a0c75a57c6a03f1ce6815421c4b8dc65` |
