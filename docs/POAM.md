...
| POAM-2026-038 | SC-4 / SI-2 | ⚠️ **Reopened 2026-08-09 — CronJob degraded.** `reconciliation-worker` CronJob fails with `CreateContainerConfigError`: `gcs-reconciliation-bucket` key in `reconciliation-worker-secrets` Secret was never populated post-deploy. `FiscalLimitGuard` is falling back to un-reconciled Redis counters — aggregate counter `fiscal:daily_limit:{day}` over-counts for up to the full daily window. Root cause (1): `RECONCILIATION_PROVIDER` was set to `"s3"` in the CronJob manifest (now corrected to `"gcs"` — aligns with `gateway.yaml` BLOCKER-06 comment). Root cause (2): `gcs-reconciliation-bucket` Secret key not populated; see `deployment/scripts/setup_reconciliation_secret.sh` for the runbook to populate it. **Secret population remains outstanding — next deploy cycle required.** Tracked in §5.2 of CAGE_ARXIV.MD. | Moderate | 2026-08-16 |
...
| POAM-2026-046 | SA-9 / External | VERITAS OS integration boundary was unspecified at the code level (`CAGE-REM-006`). Remediated by implementing `VeritasNormativeProvider` adapter in `src/integrations/veritas/` satisfying the 3-endpoint `NormativeProvider` contract and bind receipt ingestion (commit `88fa9d7`). Covered by `tests/test_veritas_integration.py`. | 2026-08-11 |
---
