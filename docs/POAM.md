...
| POAM-2026-038 | SC-4 / SI-2 | `FiscalLimitGuard` crash-leakage residual gap: per-reservation sentinel key `fiscal:reservation:{uuid}` now expires after `reservation_ttl` (300 s) so individual reservation leakage is bounded; however the aggregate counter `fiscal:daily_limit:{day}` retains the over-count until a background cleanup process subscribes to Redis keyspace-expired events and reconciles the difference — without that process, the aggregate counter over-counts for up to the full daily window. Tracked in §5.2 of CAGE_ARXIV.MD. Partial remediation committed 2026-08-06 (`src/gateway/governance/fiscal_limit_guard.py`). | Moderate | 2026-11-30 |
